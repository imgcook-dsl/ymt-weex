module.exports = function(schema, option) {
  ...
  const gradientTransValue={'90deg':'to right','180deg':'to bottom','270deg':'to left','360deg':'to top',};
  // template
  const template = [];
    ...
    for (let key in style) {
      ...
      if (boxStyleList.indexOf(key) != -1) {
        ...
        if (key == "backgroundImage") {
          value = transformGradientStyle(value)
        }
        styleData.push(`${_.kebabCase(key)}: ${value}`);
      }
    }
  ...
  //渐变色属性的处理
  const transformGradientStyle = (function (value){
    var pattern = /[\d.]+%/g;
    value = value.replace(pattern, "");
    var matchList  = value.match(/[\d.]+deg/)
    if (matchList) {
      value = value.replace(/[\d.]+deg/,gradientTransValue[matchList[0]])
    }
    return value;
  })
  ...
  const generateRender = (schema) => {
    ...
    switch (type) {
      ...
      case 'image':
        ...
        if (!source.match('"')) {
          source = `"${source}"`;
          xml = `<image${classString}${props} :src=${source} ></image> `;
        } else {
          xml = `<image${classString}${props} src=${source} ></image> `;
        }
        break;
